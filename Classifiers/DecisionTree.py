...
class decisionTreeClassifier(ClassifierInterface):
    ...
    def train(self):
        ...
        params = {'criterion': ['gini', 'entropy'], 'splitter': ['best', 'random'], 'random_state': [42, 36, 100, None],
                  'min_samples_leaf': [0.5, 1, 2], 'min_samples_split': [1, 2, 3]}
        grid_search = GridSearchCV(dtModel, param_grid=params, cv=5)
    ...
    def evaluate(self, X_train_bow, y_train_bow, best_dtModel):
        k = 10
        kfold = KFold(n_splits=k, shuffle=True, random_state=42)

        accuracy_sonuclari = cross_val_score(best_dtModel, X_train_bow, y_train_bow, cv=kfold, scoring='accuracy')
        precision_sonuclari = cross_val_score(best_dtModel, X_train_bow, y_train_bow, cv=kfold,
                                              scoring='precision_micro')
        recall_sonuclari = cross_val_score(best_dtModel, X_train_bow, y_train_bow, cv=kfold, scoring='recall_micro')
        f1_sonuclari = cross_val_score(best_dtModel, X_train_bow, y_train_bow, cv=kfold, scoring='f1_micro')
